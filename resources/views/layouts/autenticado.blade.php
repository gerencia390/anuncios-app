                            ...
                            <div class="card card-user">
                                <div class="card-body text-center" style="padding:10px;">
                                    ...
                                    <div class="box-user-menu">
                                        ...
                                        <a href="{{secure_url('logout')}}" class="dropdown-item"><i class="fa fa-sign-out"></i> Cerrar sesión</a>
                                    </div>
                                </div>
                            </div>
                            </div>
        ...
        <div class="col-md-2 nav-contextual-container collapse d-md-block" id="menuContextual">
            <div class="nav-contextual">
                    <nav class="nav nav-pills" aria-orientation="vertical">
                        <a class="nav-item nav-link @if($modulo_activo == 'dashboard'): active @endif" href="{{secure_url('dashboard')}}"><i class="fa fa-home"></i> PANEL GENERAL</a>
                        @if (Auth::user()->usu_rol == 1 || Auth::user()->usu_rol == 2 || Auth::user()->usu_rol == 3)
                        <a class="nav-item nav-link @if($modulo_activo == 'anuncios_clientes'): active @endif" href="{{secure_url('anuncios')}}"><i class="fa fa-volume-up"></i> ANUNCIOS CLIENTES</a>
                        @endif
                        @if (Auth::user()->usu_rol == 1 || Auth::user()->usu_rol == 3)
                        <a class="nav-item nav-link @if($modulo_activo == 'anuncios_propios'): active @endif" href="{{secure_url('anuncios_propios')}}"><i class="fa fa-suitcase"></i> ANUNCIOS PROPIOS</a>
                        @endif
                        @if (Auth::user()->usu_rol == 1)
                        <h6 class="text-center text-white" style="text-transform: uppercase;">
                            <small>- ADMINISTRACIÓN-</small>
                        </h6>                        
                        <a class="nav-item nav-link @if($modulo_activo == 'usuarios'): active @endif" href="{{secure_url('usuarios')}}"><i class="fa fa-users"></i> USUARIOS</a>
                        @endif
                        @if (Auth::user()->usu_rol == 1)
                        <a class="nav-item nav-link @if($modulo_activo == 'ajustes'): active @endif" href="{{secure_url('configuracion')}}"><i class="fa fa-cogs"></i> CONFIGURACION</a>
                        @endif
                    </nav>
